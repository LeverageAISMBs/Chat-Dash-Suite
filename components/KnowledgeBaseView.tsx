...
const KnowledgeBaseForm: React.FC<{ 
  onSave: (data: Omit<KnowledgeBase, 'id' | 'createdAt'>) => void; 
  onClose: () => void;
          ...
          {sources.map(s => (
            <li key={s.id} className="flex justify-between items-center bg-gray-700 p-2 rounded">
              ...
              <span className="text-xs text-gray-400">{(s.size / 1024).toFixed(2)} KB</span>
               <button onClick={() => handleRemoveSource(s.id)} className="text-red-400 hover:text-red-300 text-xs font-bold">X</button>
            </li>
          ))}
        </ul>
      </div>
...
export const KnowledgeBaseView: React.FC<KnowledgeBaseViewProps> = ({ knowledgeBases, addKnowledgeBase, updateKnowledgeBase, deleteKnowledgeBase }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingKb, setEditingKb] = useState<KnowledgeBase | null>(null);
  
  const handleSave = (data: Omit<KnowledgeBase, 'id' | 'createdAt'>) => {
    if (editingKb) {
      updateKnowledgeBase({ ...editingKb, ...data });
    } else {
    ...
    <div className="p-8 space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {knowledgeBases.map((kb) => (
          <div key={kb.id} className="bg-gray-800 p-6 rounded-lg shadow-lg hover:shadow-blue-500/20 transition-shadow duration-300 flex flex-col">
            <div className="flex-grow">
             ...
             <div className="mt-4 pt-4 border-t border-gray-700 flex space-x-2">
              <button onClick={() => openEditModal(kb)} className="flex-1 bg-gray-600 hover:bg-gray-500 text-white px-3 py-2 rounded text-sm transition-colors">Edit</button>
              <button onClick={() => handleDelete(kb.id)} className="flex-1 bg-red-600 hover:bg-red-500 text-white px-3 py-2 rounded text-sm transition-colors">Delete</button>
            </div>
          </div>
        ))}
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingKb ? "Edit Knowledge Base" : "Create New Knowledge Base"}>
        <KnowledgeBaseForm onSave={handleSave} onClose={closeModal} initialData={editingKb} />
      </Modal>
    </div>
  );
};
